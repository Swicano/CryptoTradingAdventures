...
Exchange = Cryptsy.Cryptsy('gfdsdfghjkl;', '6fgdghfgj');
tips_const = 147; #Dogecoin = 132
trade_amnt = 11000; #how many doge per trade, 100 minimum)
bias = .8; #from 0-1, pick fraction of extra money that goes toward nonBTC/LTC currency (ie in doge/btc, bias =1 puts all profits into doge)

unprocessed_trades = 0; #number of trades that have not yet been accounted for
logs= open('TIPSlog.txt' , 'a');
transacts = open('TIPStransactions.txt','a');
jasonread = open('TIPSlasttrans.json','r+');
...
while True:
	...
	while not int(myTrades['success']):
		try:
			myTrades = Exchange.myTrades(tips_const, 25); # last 100 successful  transactions
	...
	while unprocessed_trades > 0.01:
		...
		while ((not int(new_order['success'])) and ( not partial_transact)):
			try:
				new_order = Exchange.createOrder(tips_const, new_type, bias_trade_amnt , new_price);
				print ' success at new order: ';
			except:
		...
		if int(new_order['success']) or partial_transact:		
			...
			jasonfile = open('TIPSlasttrans.json','w');
