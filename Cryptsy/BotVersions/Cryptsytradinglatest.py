...
Exchange = Cryptsy.Cryptsy('fdghdfghdfgh', 'fghfghfghfgh');
markets={132:'DOGE',145:'MOON',147:'TIPS'};
originators = {0:'web',1:'this program, priority 1', 2: 'this program, priority 2'};
...
#		logs.write("New    : " + str(new_order['moreinfo']));
 	if ordercheckdelay == delay - 1:
		...
		if (web_open_orders == web_open_orders2) and (len(web_open_orders) < len(local_open_orders)):
			weborderids =set();
			for order in web_open_orders2:
				new = order['orderid'];
				weborderids.add(int(new));
			for key in local_open_orders:
				if key not in weborderids:
					del local_open_orders[key];
