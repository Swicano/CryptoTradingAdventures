...
Exchange = Cryptsy.Cryptsy('dgertyrty', 'rtyrtyrty');
markets={132:'DOGE',145:'MOON',147:'TIPS'};
marketid = 145; #Dogecoin = 132
trade_amnt = 1500; #how many doge per trade, 100 minimum)
delay = 15;
...
orders.close();
ordercheckdelay = 1; # flag to not check open orders with Cryptsy ever iteration to save time
...
#		local_open_orders = {}; #this bit worries me
	...
	while unprocessed_trades > 0.01:
		...
		#_______________________________________________ current as in the one we're focusing on, clearly it's behind
		...
		if int(current_trade['order_id']) in local_open_orders:
			local_open_orders[int(current_trade['order_id'])]['quantity'] = float(local_open_orders[int(current_trade['order_id'])]['quantity'])-float(current_trade['quantity']);
			if float(local_open_orders[int(current_trade['order_id'])]['quantity']) < partial_threshold: 
				...
				current_price = float(local_open_orders[int(current_trade['order_id'])]['price'])
				if current_type == 'Buy':
					new_type = 'Sell';
				...
				while (not int(new_order['success'])) :
					try:
						...
						if int(new_order['success']):
							print 'From current order: '+str(current_type)+' of '+str(local_open_orders[int(current_trade['order_id'])]['orig_quantity'])+' at '+str(current_price);
							logs.write('From current order: '+str(current_type)+' of '+str(local_open_orders[int(current_trade['order_id'])]['orig_quantity'])+' at '+str(current_price)+'\n');
				...
				local_open_orders.pop(int(current_trade['order_id']));
